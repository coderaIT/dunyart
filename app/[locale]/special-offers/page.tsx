...
import { getSpecialOffers } from "@/lib/queries";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    description: t("specialOffersSubtitle"),
  });
}

export default async function SpecialOffersPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations("home");
  const rugs = await getSpecialOffers();
  ...
  return (
    <div className="container-page animate-fade-up py-14">
      <div className="mb-10">
        ...
        <p className="mt-2 text-muted">{t("specialOffersSubtitle")}</p>
